...
| Processing integrity | PI1.1, PI1.4 | Result cells are carried as strings from the wire to the output, so a `DECIMAL` is never round-tripped through a floating-point type on its way to a report. Truncation by Databricks is surfaced distinctly from local display limits, so nobody reads a partial export as complete. Evidence: contract tests asserting decimals and nulls survive unchanged. |
